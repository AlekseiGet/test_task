...
// содание карточек 
    ...
    for (let i = 0; i < tokenSort.length; i++) {
        ...
        const subListNodaConteiner = document.querySelectorAll(".noda_conteiner")[searchConteiner + i];
...
function searchUser(tokenSort) {
    for (let i = 0; i < tokenSort.length; i++) {
        ...
        const tokenText = JSON.stringify(`<img src='${pictureUrl}' srcset='${picture}' alt="upss"> <a target="blank" href="https://www.google.com"> ${duble['name']} </a>`);
        let tokenItem = delLetter(tokenText, '"');
            ...
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                ...
                let tokenSort = tokenMass.sort(function (a, b) {
                    if (a.name.replace(/[0-9]/g, '') == b.name.replace(/[0-9]/g, '')) {
                        ...
                        return +a.name.replace(/[^0-9]/g, '').localeCompare(+b.name.replace(/[^0-9]/g, ''));
            ...
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                ...
                let tokenSort = tokenMass.sort(function (a, b) {
                    if (a.name.replace(/[0-9]/g, '') == b.name.replace(/[0-9]/g, '')) {
                        ...
                        return +b.name.replace(/[^0-9]/g, '').localeCompare(+a.name.replace(/[^0-9]/g, ''));
    ...
    function loadSortThree(url) {
        ...
        xhttp.onreadystatechange = function () {
            ...
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                let tokenMass = JSON.parse(xhttp.responseText);
                let tokenSort = tokenMass.sort(function (a, b) {
                    if (a['image'].create_date.replace(/[0-9]/g, '') == b['image'].create_date.replace(/[0-9]/g, '')) {

                        return +a['image'].create_date.replace(/[^0-9]/g, '').localeCompare(+b['image'].create_date.replace(/[^0-9]/g, ''));

                    } else {
                        return a['image'].create_date.localeCompare(b['image'].create_date);
                    }
            ...
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                ...
                let tokenSort = tokenMass.sort(function (a, b) {
                    if (a['image'].create_date.replace(/[0-9]/g, '') == b['image'].create_date.replace(/[0-9]/g, '')) {
                        ...
                        return +b['image'].create_date.replace(/[^0-9]/g, '').localeCompare(+a['image'].create_date.replace(/[^0-9]/g, ''));

                    } else {
                        return b['image'].create_date.localeCompare(a['image'].create_date);
                    }
...
// Бескеонечный скролл
const conteinerSlider = document.querySelector('.slider__token__conteiner'); //найти эллемент который перемещается
...
skrollKonteiner.addEventListener('scroll', function (e) {  //Событие
 ...
 const itemChaild = conteinerSlider.getBoundingClientRect().bottom;
